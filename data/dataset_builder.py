...
class DatasetBuilder:
    ...
    def build(self,databse_path:Path,output_dir:Path):
        df = self.read_sql(databse_path)
    ...
    def split_and_save(self,df:pd.DataFrame,output_dir:Path):
        ...
        X_train, X_test , y_train , y_test = train_test_split(X,y,test_size=self.config.data.test_size,stratify=df["sex"])
